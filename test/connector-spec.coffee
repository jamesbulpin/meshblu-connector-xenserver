  ...
  describe '->getCredentials', ->
    describe 'when using plaintext credentials', ->
      ...
      it 'should return the correct credentials', ->
        ...
        expect(creds.user).to.be.equal('testusername')
    ...
    describe 'when using encrypted credentials', ->
      beforeEach (done) ->
        ...
        key = new NodeRSA testkeyprv, "private"
        tmpobj = tmp.fileSync()
        fs.writeFileSync tmpobj.fd, testkeypub
        fs.closeSync tmpobj.fd
        @sut.onConfig { options: { username: key.encryptPrivate("testusername", 'base64'), password: key.encryptPrivate("testpassword", 'base64'), credskey:tmpobj.name } }
        done()
