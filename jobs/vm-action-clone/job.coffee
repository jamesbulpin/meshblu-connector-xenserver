http = require 'http'
debug = require('debug')('meshblu-connector-xenserver:VmActionClone')

class VmActionClone
  constructor: ({@connector}) ->
    throw new Error 'VmActionClone requires connector' unless @connector?
  ...
  do: ({data}, callback) =>
    ...
    @connector.getXapi().then((xapi) =>
      xapi.call('VM.get_by_name_label', name).then((vmref) =>
        ...
        xapi.call('VM.clone', vmref[0], name + " Clone by Octoblu").then((response) =>
      
          message = "VM Clone on " + name
          status = "ok"
          data = {message, status}
