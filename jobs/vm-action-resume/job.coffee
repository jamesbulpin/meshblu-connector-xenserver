http = require 'http'
debug = require('debug')('meshblu-connector-xenserver:VmResume')

class VmResume
  constructor: ({@connector}) ->
    throw new Error 'VmResume requires connector' unless @connector?
  ...
  do: ({data}, callback) =>
    ...
    @connector.getXapi().then((xapi) =>
      xapi.call('VM.get_by_name_label', name).then((vmref) =>
        ...
        xapi.call('VM.resume', vmref[0], false, false).then((response) =>
      
          message = "Resumed VM " + name
          status = "ok"
          data = {message, status}
