http = require 'http'
debug = require('debug')('meshblu-connector-xenserver:VmSnapshot')

class VmSnapshot
  constructor: ({@connector}) ->
    throw new Error 'VmSnapshot requires connector' unless @connector?
  ...
  do: ({data}, callback) =>
    ...
    @connector.getXapi().then((xapi) =>
      xapi.call('VM.get_by_name_label', name).then((vmref) =>
        ...
        xapi.call('VM.snapshot', vmref[0], name + " Snap by Octoblu").then((response) =>
