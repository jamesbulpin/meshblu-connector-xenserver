http = require 'http'
debug = require('debug')('meshblu-connector-xenserver:VmSuspend')

class VmSuspend
  constructor: ({@connector}) ->
    throw new Error 'VmSuspend requires connector' unless @connector?
  ...
  do: ({data}, callback) =>
    ...
    @connector.getXapi().then((xapi) =>
      xapi.call('VM.get_by_name_label', name).then((vmref) =>
        ...
        xapi.call('VM.suspend', vmref[0]).then((response) =>
      
          message = "Suspended VM " + name
          status = "ok"
          data = {message, status}
